...
export default function SubCategoryPage() {
  ...
  const subC = getSubColor(subName);

  return (
    <div className="min-h-screen">
      {/* Header */}
      <section className={`py-10 sm:py-12 ${subC.bg} border-b ${subC.border}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-sm mb-5 flex-wrap">
            ...
            <Link href={`/category/${encodeURIComponent(categoryName)}`} className={`${catC.text} hover:text-primary-500 transition-colors`}>
              {categoryName}
            </Link>
            <span className="text-gray-400">/</span>
            <span className={`font-medium ${subC.text}`}>{subName}</span>
          </nav>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              <div className={`w-2 h-8 rounded-full ${catC.bar}`} />
              <div className={`w-2 h-8 rounded-full ${subC.bar}`} />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${catC.badge}`}>
                  {categoryName}
                </span>
                <span className="text-gray-400">›</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-surface dark:bg-gray-900 rounded-xl p-4 mb-8 border border-gray-200 dark:border-gray-800">
          <p className="text-center text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Summary Detail Level</p>
          <ContentSlider />
        </div>
        ...
        {!loading && articles.length === 0 && (
          <div className="text-center py-16">
            ...
            <Link href={`/category/${encodeURIComponent(categoryName)}`} className="text-primary-500 hover:text-primary-600 mt-4 inline-block">
